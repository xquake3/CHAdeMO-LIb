...
volatile uint32_t test_cgc = 0;

#pragma inline_asm jump_f2
void jump_f2(void){JMP 0x00000000}
...
void R_CGC_Create(void)
{
    ...
    CLKCTL.CKSC_AWDTAD_STPM = _CGC_CKSC_AWDTAD_STPM_DEFAULT_VALUE | _CGC_WDTA_CLK_REQUEST_CONTINUE;
    ...
    WPROTR.PROTCMD0 = _WRITE_PROTECT_COMMAND;
    CLKCTL.CKSC_ARTCAS_CTL = _CGC_RTCA_CLK_SOURCE_LSOSC;
    CLKCTL.CKSC_ARTCAS_CTL = (uint32_t) ~_CGC_RTCA_CLK_SOURCE_LSOSC;
    CLKCTL.CKSC_ARTCAS_CTL = _CGC_RTCA_CLK_SOURCE_LSOSC;
    while (CLKCTL.CKSC_ARTCAS_ACT != _CGC_RTCA_CLK_SOURCE_LSOSC)
    {
        NOP();
    }
    WPROTR.PROTCMD0 = _WRITE_PROTECT_COMMAND;
    CLKCTL.CKSC_ARTCAD_CTL = _CGC_RTCA_CLK_DIVIDER_1;
    CLKCTL.CKSC_ARTCAD_CTL = (uint32_t) ~_CGC_RTCA_CLK_DIVIDER_1;
    CLKCTL.CKSC_ARTCAD_CTL = _CGC_RTCA_CLK_DIVIDER_1;
    while (CLKCTL.CKSC_ARTCAD_ACT != _CGC_RTCA_CLK_DIVIDER_1)
    {
        NOP();
    }
    CLKCTL.CKSC_ARTCAD_STPM = _CGC_CKSC_ARTCAD_STPM_DEFAULT_VALUE | _CGC_RTCA_CLK_REQUEST_STOP;
    /* ADCA0 clock domain setting */
    WPROTR.PROTCMD0 = _WRITE_PROTECT_COMMAND;
    CLKCTL.CKSC_AADCAS_CTL = _CGC_ADCA0_CLK_SOURCE_MAINOSC;
    CLKCTL.CKSC_AADCAS_CTL = (uint32_t) ~_CGC_ADCA0_CLK_SOURCE_MAINOSC;
    CLKCTL.CKSC_AADCAS_CTL = _CGC_ADCA0_CLK_SOURCE_MAINOSC;
    while (CLKCTL.CKSC_AADCAS_ACT != _CGC_ADCA0_CLK_SOURCE_MAINOSC)
    {
        NOP();
    }
    WPROTR.PROTCMD0 = _WRITE_PROTECT_COMMAND;
    ...
    WPROTR.PROTCMD1 = _WRITE_PROTECT_COMMAND;
    CLKCTL.CKSC_IADCAS_CTL = _CGC_ADCA1_CLK_SOURCE_PPLLCLK2;
    CLKCTL.CKSC_IADCAS_CTL = (uint32_t) ~_CGC_ADCA1_CLK_SOURCE_PPLLCLK2;
    CLKCTL.CKSC_IADCAS_CTL = _CGC_ADCA1_CLK_SOURCE_PPLLCLK2;
    while (CLKCTL.CKSC_IADCAS_ACT != _CGC_ADCA1_CLK_SOURCE_PPLLCLK2)
    {
        NOP();
    }
    WPROTR.PROTCMD1 = _WRITE_PROTECT_COMMAND;
    CLKCTL.CKSC_IADCAD_CTL = _CGC_ADCA1_CLK_DIVIDER_1;
    CLKCTL.CKSC_IADCAD_CTL = (uint32_t) ~_CGC_ADCA1_CLK_DIVIDER_1;
    CLKCTL.CKSC_IADCAD_CTL = _CGC_ADCA1_CLK_DIVIDER_1;
    while (CLKCTL.CKSC_IADCAD_ACT != _CGC_ADCA1_CLK_DIVIDER_1)
    {
        NOP();
    }
    ...
    CLKCTL.CKSC_ICANS_STPM = _CGC_CKSC_ICANS_STPM_DEFAULT_VALUE | _CGC_RSCAN_CLK_REQUEST_CONTINUE;
    WPROTR.PROTCMD1 = _WRITE_PROTECT_COMMAND;
