...
#define set_pin( map_pin , val ) {  if( (val) == 0) { map_pin##_OF ; } else {  map_pin##_ON ; } }

#define LED_OUT_ON PORT.P0|=1<<10
#define LED_OUT_OF PORT.P0&=~(1<<10)

#define BRAKE_L_UC_OUT_ON PORT.P11|=1<<12 // 10
#define BRAKE_L_UC_OUT_OF PORT.P11&=~(1<<12) // 10

#define POSITION_UC_OUT_ON PORT.P11|=1<<13 // 11
#define POSITION_UC_OUT_OF PORT.P11&=~(1<<13)

#define TSL_R_UC_OUT_ON PORT.P11|=1<<14 //12
#define TSL_R_UC_OUT_OF PORT.P11&=~(1<<14)

#define IR_UC_OUT_ON PORT.P8|=1<<11 // 32			<-------|TSL_L_UC_OUT_ON
#define IR_UC_OUT_OF PORT.P8&=~(1<<11)	//			<-------|TSL_L_UC_OUT_OF
						//			|
#define HEADLAMP_HB_UC_OUT_ON PORT.P1|=1<<2 // 27			|TSL_Left NOT NOT NOT
#define HEADLAMP_HB_UC_OUT_OF PORT.P1&=~(1<<2)	//			|SWAPPED
						//			|   TO
#define HEADLAMP_LB_UC_OUT_ON PORT.P1|=1<<3 // 28			| IR_OUT	
#define HEADLAMP_LB_UC_OUT_OF PORT.P1&=~(1<<3)	//			|  DUE
						//			|   to
#define CLUSTER_UC_OUT_ON PORT.P8|=1<<10 // 31	//			|CONNECTOR
#define CLUSTER_UC_OUT_OF PORT.P8&=~(1<<10)	//			|It 
						//			|is originol
#define TSL_L_UC_OUT_ON PORT.P0|=1<<6 // 20//PORT.P8|=1<<11 //32<-------|here
#define TSL_L_UC_OUT_OF PORT.P0&=~(1<<6)//PORT.P8&=~(1<<11)	<-------|

#define HORN_UC_OUT_ON PORT.P11|=1<<1 //129
#define HORN_UC_OUT_OF PORT.P11&=~(1<<1)

#define SPARE_UC_OUT_ON PORT.P1|=1<<1 // 26
#define SPARE_UC_OUT_OF PORT.P1&=~(1<<1)

#define SPARE3_UC_OUT_ON PORT.P10|=1<<8 // 122
#define SPARE3_UC_OUT_OF PORT.P10&=~(1<<8)

#define PWM_OUT_UC_OUT_ON PORT.P8|=1<<12 // 33
#define PWM_OUT_UC_OUT_OF PORT.P8&=~(1<<12)											  

#define SCL_ON PORT.P10|=(1<<3) //1
#define SCL_OF PORT.P10&=~(1<<3)

#define IPD_SENSE_EN_ON PORT.P10|=(1<<15) //4
#define IPD_SENSE_EN_OF PORT.P10&=~(1<<15)

#define CP_PP_ON PORT.P10|=(1<<8) //122
#define CP_PP_OF PORT.P10&=~(1<<8)

//#define IR_SELECT_OUT_ON PORT.P11|=(1<<9) //7
//#define IR_SELECT_OUT_OF PORT.P10&=~(1<<3) 

#define WDT_CLK_OUT_ON PORT.P11|=(1<<10) //8
#define WDT_CLK_OUT_OF PORT.P11&=~(1<<10) //8

#define WDT_EN_OUT_ON PORT.P11|=(1<<11) //9
#define WDT_EN_OUT_OF PORT.P11&=~(1<<11) //9

//#define BMS_UC_OUT_ON PORT.P1|=(1<<6) //61
//#define BMS_UC_OUT_OF PORT.P1&=~(1<<6) //61 //NU

//#define BMS_BYPAS_OUT_ON PORT.P11|=(1<<2) //130
//#define BMS_BYPAS_OUT_OF PORT.P11&=~(1<<2) //130 //NU

#define BMS_IGN_OUT_UC_ON PORT.P1|=(1<<5) //62 // Named as BMS_IGN_OUT_UC in PCB , use to switch on bms 
#define BMS_IGN_OUT_UC_OF PORT.P1&=~(1<<5) //62

//#define CHARGE_UC_OUT_ON PORT.P1|=(1<<4) //63 Not in use
//#define CHARGE_UC_OUT_OF PORT.P1&=~(1<<4) //63

//#define DCDC_UC_OUT_ON PORT.P8|=(1<<0) //64 Not in use
//#define DCDC_UC_OUT_OF PORT.P8&=~(1<<0) //64 //NU

#define EXTINT_UC_OUT_ON PORT.P8|=(1<<1) //65
#define EXTINT_UC_OUT_OF PORT.P8&=~(1<<1) //65

#define ADC_REF_OUT_ON PORT.AP0|=(1<<15)
#define ADC_REF_OUT_OF PORT.AP0&=~(1<<15) //NU

#define CAN_STBY_OUT_ON PORT.P11 |= (1<<9);	// CAN_STBY pin P11_9
#define CAN_STBY_OUT_OF PORT.P11 &= ~(1<<9);

//========================== INPUT ================================//																	 
#define KILL_SW_IN_  ((PORT.PPR10 & (1<<12))==0) //126

#define IGNITION_SW_OF_IN_ ((PORT.PPR9&(1<<5))==0) // 97

#define IGNITION_SW_ON_IN_ ((PORT.PPR8&(1<<3))==0) //66

#define STARTER_SW_IN_ ((PORT.PPR9&(1<<4))==0) //96

#define SIDESTAND_SW_IN_ ((PORT.PPR1&(1<<9))==0) //42

#define T_CNTR_INT_IN_ ((PORT.PPR8&(1<<2))==0) //30

#define VP_INT_IN_ ((PORT.PPR0&(1<<13))==0) //23 

#define CHRG_INT_2_IN_ ((PORT.PPR1&(1<<6))==0) //23

#define AUX_UV_SW_IN_ ((PORT.PPR8&(1<<4))>0) //67

#define CHARGE_SW_IN_ ((PORT.PPR8&(1<<5))==0) //68

#define PASS_BY_SW_IN_ ((PORT.PPR8&(1<<8)) == 0 ) //71

#define HAZARD_SW_IN_ ((PORT.PPR8&(1<<9))==0) //72 //HOLIDAY_SW_IN_

#define CITY_SW_IN_ ((PORT.PPR9&(1<<0))==0) //92

#define SPORT_SW_IN_ ((PORT.PPR9&(1<<1))==0) //93

#define ECO_SW_IN_ ((PORT.PPR9&(1<<2))==0) //94

#define THROTTLE_SW_IN_ ((PORT.PPR9&(1<<3))>0) //95

#define TSL_R_SW_IN_ ((PORT.PPR20&(1<<5))==0) //99

#define TSL_L_SW_IN_ ((PORT.PPR20&(1<<4))==0) //100

#define HL_LB_SW_IN_ ((PORT.PPR18&(1<<0))==0) //115

#define HL_HB_SW_IN_ ((PORT.PPR18&(1<<1))==0) //116

#define BRAKE_FR_SW_IN_ ((PORT.PPR18&(1<<2))==0) //117

#define HORN_SW_IN_ ((PORT.PPR18&(1<<3))==0) //118

#define SELECT_SW_IN_ ((PORT.PPR10&(1<<13))==0) //127

#define BRAKE_RE_SW_IN_ ((PORT.PPR11&(1<<5))==0) //133

#define AUX_UNDERVOLTAGE_IN_ ((PORT.PPR8 & (1<<4)) == (1<<4))
